# FlowDo project state

Development reference and verification history. For the quick Mac setup, see
[README.md](README.md). Commands below run from the repository root unless noted.
...
The badge starts at 180 pixels wide. Drag its top handle to move it, or its
lower-right grip to resize it. Task text wraps; long content can be scrolled.
Movement and sizing are retained while FlowDo runs. Choosing a different corner
in Preferences repositions the badge.
...
clock. Pulse stops with Pause, automatic break, an empty task, hidden badge, or Reduce Motion.
...
contains 42 native tests, including recurring intervals, time aggregation, logging
consent, paired spans, file failure/recovery, exact idle grace/frozen timers,
pulse averaging/phase, fixed three-second swirl endpoints, and progressive idle
retreat through the automatic break state. Four Python activity-report
